...
public sealed record UserResponse(Guid Id, string Account, string Email, string? Name, bool EmailConfirmed,
    bool IsEnabled, string Role, bool IsBootstrapAdmin);
public sealed record UpdateRoleRequest(Guid RoleId);
public sealed record UpdateUserStatusRequest(bool IsEnabled);
public sealed record UpdateAdministrationRequest(Guid RoleId, bool IsEnabled);
